...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetA(double psi,
                                                                    double dt,
                                                                    bool update_linearization,
                                                                    Eigen::Ref<AMatrixT> A) {
  if (!update_linearization) {
    A.setIdentity();
    A.template block<3, 3>(3, 9).diagonal().setConstant(dt);
    A(11, 12) = -dt;
  }
  MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetR(psi, dt, update_linearization,  A.template block<3, 3>(0, 6));
}

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetB(
    double psi,
    double m,
  ...
  if (!update_linearization) {
    ...
    for (unsigned int foot_indx = 0; foot_indx < NUM_FEET; foot_indx++) {
      B.template block<3, 3>(9, foot_indx * 3).diagonal().setConstant(dt / m);
    }
  }
  Eigen::Matrix3d I_rot;
  MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetIWorld(psi, inertia, I_rot);
  Eigen::Matrix3d I_rot_inv = I_rot.inverse();
  for (unsigned int foot_indx = 0; foot_indx < NUM_FEET; foot_indx++) {
    if (contacts[foot_indx]) {
      B.template block<3, 3>(6, foot_indx * 3) = I_rot_inv * skew(r[foot_indx]) * dt;
    } else {
      B.template block<3, 3>(6, foot_indx * 3).setZero();
    }
  }
}

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetR(double psi,
                                                                    double multiply_with,
                                                                    bool update_linearization,
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetIWorld(
    double psi, const Eigen::Ref<const Eigen::Matrix3d>& inertia, Eigen::Ref<Eigen::Matrix3d> IWorld) {
  static Eigen::Matrix3d R;
  MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetR(psi, 1.0, false, R);
  IWorld.noalias() = R * inertia * R.transpose();
}

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetC(
    double mu, Eigen::Ref<Eigen::Matrix<double, NUM_POLY_CONST_PER_FOOT * NUM_FEET, INPUT_SIZE>> C) {
  for (unsigned int foot = 0; foot < NUM_FEET; foot++) {
    C.template block<NUM_POLY_CONST_PER_FOOT, 3>(NUM_POLY_CONST_PER_FOOT * foot, 3 * foot) << 1, 0, -mu, 1, 0, mu, 0, 1, -mu, 0,
        1, mu;
  }
}

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetCbounds(
    double fmin,
    double fmax,
  ...
  (void)fmax;
  for (unsigned int foot = 0; foot < NUM_FEET; foot++) {
    auto block_lb = c_lb.template block<NUM_POLY_CONST_PER_FOOT, 1>(foot * NUM_POLY_CONST_PER_FOOT, 0);
    auto block_ub = c_ub.template block<NUM_POLY_CONST_PER_FOOT, 1>(foot * NUM_POLY_CONST_PER_FOOT, 0);
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetUBounds(const std::array<bool, NUM_FEET>& contact,
                                                                          double fmin,
                                                                          double fmax,
  ...
  for (unsigned int feet_idx = 0; feet_idx < NUM_FEET; feet_idx++) {
    auto lb_block = lb.template block<3, 1>(feet_idx * 3, 0);
    auto ub_block = ub.template block<3, 1>(feet_idx * 3, 0);
    if (contact[feet_idx]) {
      lb_block(0) = -fmax;
      lb_block(1) = -fmax;
      lb_block(2) = fmin;
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::MPC(
    double alpha,
  ...
  for (unsigned int k = 0; k < PREDICTION_HORIZON; k++) {
    MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetA(0, DeltaT, false, As_[k]);
    MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetB(0,
                                                                   quad_model_->GetMass(),
  ...
  // Constants
  MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetC(mu_, D_);
  MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetCbounds(fmin_, fmax_, Clb_, Cub_);
  Eigen::Matrix<double, STATE_SIZE, 1> state_weights_with_g;
  state_weights_with_g.setConstant(0);
  state_weights_with_g.template block<STATE_SIZE - 1, 1>(0, 0) = state_weights;
  ...
  if (solver == PARTIAL_CONDENSING_OSQP) {
    ...
    int polish = 0;
    ocp_qp_xcond_solver_opts_set(
        solver_config_, reinterpret_cast<ocp_qp_xcond_solver_opts*>(solver_opts_), "osqp_polish", &polish);
  }
#else
  ...
  if (solver_tolerances > 0.0) {
    for (auto tol_name : {"tol_dual_gap", "tol_eq", "tol_stat", "tol_ineq", "tol_comp"}) {
      std::cout << "Set tolerance " << tol_name << " to " << solver_tolerances << std::endl;
      ocp_qp_xcond_solver_opts_set(
  ...
  blasfeo_allocate_dvec(ocp_dims_->nx[0] + ocp_dims_->nu[0] + 2 * ocp_dims_->ns[0], &nv_temp_);
...
MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::~MPC() {
  ...
  blasfeo_free_dvec(&nv_temp_);
}

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::UpdateState(const StateInterface& quad_state) {
  *quad_state_ = quad_state;
};

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::UpdateModel(const ModelInterface& quad_model) {
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::UpdateGaitSequence(const GaitSequence& gait_sequence) {
  gait_sequence_ = gait_sequence;
}

template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::SetStateWeights(
    const Eigen::Matrix<double, STATE_SIZE - 1, 1>& weights) {
  Q_.diagonal().template block<STATE_SIZE - 1, 1>(0, 0) = weights;
  for (int k = 0; k <= PREDICTION_HORIZON; k++) {
    ocp_qp_in_set(solver_config_, qp_in_, k, const_cast<char*>("Q"), Q_.data());
  }
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::SetInputWeights(double alpha) {
  R_ = RMatrixT::Identity() * alpha;
  for (int k = 0; k < PREDICTION_HORIZON; k++) {
    ocp_qp_in_set(solver_config_, qp_in_, k, const_cast<char*>("R"), R_.data());
  }
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::SetFmax(double fmax) {
  fmax_ = fmax;
  for (int k = 0; k < PREDICTION_HORIZON; k++) {
    MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetUBounds(
        gait_sequence_.contact_sequence[k], fmin_, fmax_, ulbs_[k], uubs_[k]);
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::SetMu(double mu) {
  mu_ = mu;
  MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetC(mu_, D_);
  for (int k = 0; k < PREDICTION_HORIZON; k++) {
    ocp_qp_in_set(solver_config_, qp_in_, k, const_cast<char*>("D"), D_.data());
...
template <size_t NUM_FEET, size_t PREDICTION_HORIZON, size_t MPC_DT_NS, size_t STATE_SIZE>
void MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetWrenchSequence(
    WrenchSequenceInterface* wrench_sequence_ptr,
    MPCPredictionInterface* state_prediction_ptr,
    SolverInformation& solver_information) {
  WrenchSequence<NUM_FEET, PREDICTION_HORIZON>& wrench_sequence =
      *static_cast<WrenchSequence<NUM_FEET, PREDICTION_HORIZON>*>(wrench_sequence_ptr);

  MPCPrediction<PREDICTION_HORIZON, STATE_SIZE>& state_prediction =
      *static_cast<MPCPrediction<PREDICTION_HORIZON, STATE_SIZE>*>(state_prediction_ptr);
  ...
  for (int k = 0; k <= PREDICTION_HORIZON; k++) {
    ...
    if (k < PREDICTION_HORIZON) {  // Input related stuff is only up to PREDICITION HORIZON - 1
      std::array<const Eigen::Ref<const Eigen::Vector3d>, 4> rs{
          ...
          gait_sequence_.foot_position_sequence[k][3]
              - (gait_sequence_.desired_reference_trajectory_position[k] + base_link_to_com)};
      MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetA(psi_avg, DeltaT, true, As_[k]);
      MPC<NUM_FEET, PREDICTION_HORIZON, MPC_DT_NS, STATE_SIZE>::GetB(desired_yaw,
                                                                     quad_model_->GetMass(),
                                                                     quad_model_->GetInertia(),
                                                                     rs,
    ...
    target_state.template block<3, 1>(0, 0) =
    ...
    target_state.template block<3, 1>(3, 0) = gait_sequence_.desired_reference_trajectory_position[k] + base_link_to_com;
    target_state.template block<3, 1>(6, 0) = gait_sequence_.reference_trajectory_twist[k];
    target_state.template block<3, 1>(9, 0) = gait_sequence_.reference_trajectory_velocity[k];
    target_state(12, 0) = 0;
  ...
  // Set current state:
  x0_.template block<3, 1>(0, 0) = quaternion_to_euler(quad_state_->GetOrientationInWorld());
  x0_.template block<3, 1>(3, 0) = quad_state_->GetPositionInWorld() + base_link_to_com;
  x0_.template block<3, 1>(6, 0) = quad_state_->GetAngularVelInWorld();
  x0_.template block<3, 1>(9, 0) = quad_state_->GetLinearVelInWorld();
  ...
  if (acados_return == ACADOS_SUCCESS) {
    ...
    for (int k = 0; k <= PREDICTION_HORIZON; k++) {
      ...
      state_prediction.orientation[k] = euler_to_quaternion<double>(state_prediction_raw[k].template block<3, 1>(0, 0));
      state_prediction.position[k] = state_prediction_raw[k].template block<3, 1>(3, 0) - base_link_to_com;
      state_prediction.angular_velocity[k] = state_prediction_raw[k].template block<3, 1>(6, 0);
      state_prediction.linear_velocity[k] = state_prediction_raw[k].template block<3, 1>(9, 0);
    }
    // Get objective value
    solver_information.qp_objective_value = 0.0;
    int i, nux, ns;
    int N = ocp_dims_->N;

    // Sum over stages 0 to N
    for (i = 0; i <= N; i++) {
      nux = ocp_dims_->nx[i] + ocp_dims_->nu[i];
      ns = ocp_dims_->ns[i];

      // Calculate 0.5 * d.T H d
      blasfeo_dsymv_l(nux, 0.5, &qp_in_->RSQrq[i], 0, 0, &qp_out_->ux[i], 0, 0.0, &qp_out_->ux[i], 0, &nv_temp_, 0);
      solver_information.qp_objective_value += blasfeo_ddot(nux, &qp_out_->ux[i], 0, &nv_temp_, 0);

      // slack QP objective value, compare to computation in cost modules;
      // tmp_nv = 2 * z + Z .* slack;
      blasfeo_dveccpsc(2 * ns, 2.0, &qp_out_->ux[i], nux, &nv_temp_, 0);
      blasfeo_dvecmulacc(2 * ns, &qp_in_->Z[i], 0, &qp_out_->ux[i], nux, &nv_temp_, 0);
      // qp_cost += .5 * (tmp_nv .* slack)
      solver_information.qp_objective_value += 0.5 * blasfeo_ddot(2 * ns, &nv_temp_, 0, &qp_out_->ux[i], nux);
      // Calculate g.T d
      solver_information.qp_objective_value += blasfeo_ddot(nux, &qp_out_->ux[i], 0, &qp_in_->rqz[i], 0);

      // Calculate gradient of slacks
      solver_information.qp_objective_value += blasfeo_ddot(2 * ns, &qp_out_->ux[i], nux, &qp_in_->rqz[i], nux);
    }
    ocp_qp_inf_norm_residuals(ocp_dims_, qp_in_, qp_out_, solver_information.qp_residuals);
  }
  model_update_ = false;
}
