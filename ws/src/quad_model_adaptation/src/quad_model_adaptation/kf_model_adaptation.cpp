#include "quad_model_adaptation/kf_model_adaptation.hpp"

KFModelAdaptation::KFModelAdaptation(
    std::unique_ptr<ModelInterface> quad_model,
    std::unique_ptr<StateInterface> initial_state,
    const Eigen::Ref<const Eigen::Matrix<double, NUM_PARAMS, NUM_PARAMS>> process_noise,
    const Eigen::Ref<const Eigen::Matrix<double, 6, 6>> measurement_noise,
    const double GRAVITY_CONSTANT,
    const Eigen::Ref<const Eigen::Vector<double, NUM_PARAMS>> CONV_THRESH)
    : state_(std::move(initial_state)),
      conv_thresh_(CONV_THRESH),
      process_noise_(process_noise),
      measurement_noise_(measurement_noise),
      g_(GRAVITY_CONSTANT) {
  assert(NUM_PARAMS == 3);
  // initialize parameter vector
  // mass
  parameter_vector_(0) = quad_model->GetMass();
  // com
  auto com = quad_model->GetBodyToCOM();
  parameter_vector_(1) = com.x() * parameter_vector_(0);
  parameter_vector_(2) = com.y() * parameter_vector_(0);

  // initialize parameter covariance
  parameter_covariance_.setIdentity();  // TODO: better initialization?

  // get leg total mass and com in zeropos
  leg_total_mass_ = 0.0;
  leg_total_mcom_ = Eigen::Vector3d::Zero();

  auto joint_state = state_->GetJointPositions();
  std::array<Eigen::Vector3d, 4> joint_pos;
  for (int i = 0; i < 4; i++) {
    double leg_mass = quad_model->GetLegMass(i);
    Eigen::Translation3d leg_com = quad_model->GetBodyToLegCOM(i);
    joint_pos[i] << joint_state[i][0], joint_state[i][1], joint_state[i][2];
    leg_total_mass_ += leg_mass;
    leg_total_mcom_ += leg_mass * leg_com.translation();
  }
}

void KFModelAdaptation::UpdateState(const StateInterface& state) { *state_ = state; }

void KFModelAdaptation::UpdateGaitSequence(const GaitSequence& gs) { gait_sequence_ = gs; }

bool KFModelAdaptation::DoModelAdaptation(ModelInterface& model) {
  // populate measurement vector
  std::array<bool, 4> contact_state = state_->GetFeetContacts();
  std::array<Eigen::Vector3d, 4> foot_positions;
  std::array<Eigen::Vector3d, 4> contact_forces = state_->GetContactForces();
  for (int i = 0; i < 4; i++) {
    foot_positions[i] =
        model.CalcFootPositionInBodyFrame(i, Eigen::Map<const Eigen::Vector3d>(state_->GetJointPositions()[i].data()));
    contact_forces[i] = state_->GetOrientationInWorld().inverse() * contact_forces[i];
  }

  CalcMeasurementVector(
      contact_forces, foot_positions, contact_state, gait_sequence_.contact_sequence[0], measurement_vector_);

  //  get regressor
  Eigen::Matrix<double, 18, 130> full_body_regressor;
  model.ComputeRegressorMatrix(*state_, full_body_regressor);
  Eigen::Matrix<double, 6, NUM_PARAMS> base_link_regressor = full_body_regressor.block<6, NUM_PARAMS>(0, 0);

  // do KF prediction
  DoPredictionStep(process_noise_, parameter_covariance_, true);
  // do KF update
  DoUpdateStep(base_link_regressor, measurement_noise_, measurement_vector_, parameter_vector_, parameter_covariance_);
  // check if mass is to be updated
  Eigen::Array<bool, NUM_PARAMS, 1> updated;
  updated.setConstant(false);
  if (parameter_covariance_(0, 0) < conv_thresh_(0)) {
    model.SetMass(parameter_vector_(0) - leg_total_mass_);
    updated(0) = true;
  }
  // check which com parameters are set
  // if one param is not selected and covariance is not low enough don't change
  Eigen::Vector3d set_com = model.GetBodyToCOM().translation() * model.GetMass();
  for (uint i = 1; i < 3; i++) {
    if (parameter_covariance_(i, i) < conv_thresh_(i)) {
      set_com(i - 1) = parameter_vector_(i);
      updated(i) = true;
    }
  }
  if ((updated.segment<2>(1) == true).any()) {
    Eigen::Vector3d com_trunk;
    com_trunk = (set_com - leg_total_mcom_) / parameter_vector_(0);
    model.SetCOM(com_trunk);
  }

  if ((updated == true).any()) {
    return true;
  } else {
    return false;
  }
  return false;
}

void KFModelAdaptation::DoPredictionStep(
    const Eigen::Ref<const Eigen::Matrix<double, NUM_PARAMS, NUM_PARAMS>> process_noise,
    Eigen::Ref<Eigen::Matrix<double, NUM_PARAMS, NUM_PARAMS>> parameter_covariance,
    bool full_id) const {
  if (full_id) {
    parameter_covariance = (parameter_covariance + process_noise).eval();
  } else {
    parameter_covariance.block<3, 3>(0, 0) =
        (parameter_covariance.block<3, 3>(0, 0) + process_noise.block<3, 3>(0, 0)).eval();
  }
}

void KFModelAdaptation::DoUpdateStep(
    const Eigen::Matrix<double, 6, NUM_PARAMS>& regressor_matrix,
    const Eigen::Ref<const Eigen::Matrix<double, 6, 6>> measurement_noise,
    const Eigen::Ref<const Eigen::Vector<double, 6>> measurement_vector,
    Eigen::Ref<Eigen::Vector<double, NUM_PARAMS>> parameter_vector,
    Eigen::Ref<Eigen::Matrix<double, NUM_PARAMS, NUM_PARAMS>> parameter_covariance) const {
  Eigen::Matrix<double, NUM_PARAMS, 6> kalman_gain =
      parameter_covariance * regressor_matrix.transpose()
      * (regressor_matrix * parameter_covariance * regressor_matrix.transpose() + measurement_noise).inverse();
  parameter_vector =
      (parameter_vector + kalman_gain * (measurement_vector - regressor_matrix * parameter_vector)).eval();
  Eigen::Matrix<double, NUM_PARAMS, NUM_PARAMS> identity;
  identity.setIdentity();
  parameter_covariance = ((identity - (kalman_gain * regressor_matrix)) * parameter_covariance).eval();
}

void KFModelAdaptation::CalcMeasurementVector(const std::array<Eigen::Vector3d, 4>& contact_forces,
                                              const std::array<Eigen::Vector3d, 4>& foot_positions,
                                              const std::array<bool, 4>& contact_state,
                                              const std::array<bool, 4>& planned_contact,
                                              Eigen::Ref<Eigen::Vector<double, 6>> measurement_vector) const {
  // sum of forces acting on torso
  Eigen::Vector3d forces = Eigen::Vector3d::Zero();
  Eigen::Vector3d torques = Eigen::Vector3d::Zero();
  for (int i = 0; i < 4; i++) {
    if (contact_state[i] && planned_contact[i]) {
      forces += contact_forces[i];
      torques += foot_positions[i].cross(contact_forces[i]);
    }
  }
  measurement_vector.head<3>() = forces;   // set first 3 elements to forces
  measurement_vector.tail<3>() = torques;  // and last 3 to torques
}

Eigen::Vector<double, KFModelAdaptation::NUM_PARAMS> KFModelAdaptation::GetParameterVector() const {
  return parameter_vector_;
}

Eigen::Vector<double, KFModelAdaptation::NUM_PARAMS> KFModelAdaptation::GetDelta() const {
  return Eigen::Vector<double, NUM_PARAMS>::Zero();
}

Eigen::Matrix<double, KFModelAdaptation::NUM_PARAMS, KFModelAdaptation::NUM_PARAMS>
KFModelAdaptation::GetParameterCovariance() const {
  return parameter_covariance_;
}

Eigen::Vector<double, 6> KFModelAdaptation::GetTotalForceTorque() const { return measurement_vector_; }

Eigen::Vector<double, KFModelAdaptation::NUM_PARAMS> KFModelAdaptation::GetSV() const {
  return Eigen::Vector<double, NUM_PARAMS>::Zero();
}