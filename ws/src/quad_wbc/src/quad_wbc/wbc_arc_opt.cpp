...
WBCArcOPT::WBCArcOPT(std::unique_ptr<StateInterface> state,
  ...
  if (wbc_joint_names.size() != ModelInterface::NUM_JOINTS) {
    throw std::runtime_error("Mismatch between WBC joint names and ModelInterface joint count");
  }
...
void WBCArcOPT::UpdateModel(const ModelInterface& quad_model) {
  ...
  auto idx = wbc_model_temp->getFrameId("base_link");
  if (idx != wbc_model_temp->frames.size()) {
    ...
    throw std::runtime_error("base_link frame not found in wbc model");
  }
}
