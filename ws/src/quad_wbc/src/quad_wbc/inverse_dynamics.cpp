...
InverseDynamics::InverseDynamics(std::unique_ptr<ModelInterface> model,
    ...
    : quad_model_(std::move(model)),
      ...
      body_velocity_filter_(std::max(transformation_filter_size, 1u)),
      body_twist_filter_(std::max(transformation_filter_size, 1u)) {
...
void InverseDynamics::setTransformationFilterSize(unsigned int filter_size) {
  const auto clamped_filter_size = std::max(filter_size, 1u);
  body_twist_filter_.resize(clamped_filter_size);
  body_velocity_filter_.resize(clamped_filter_size);
}
